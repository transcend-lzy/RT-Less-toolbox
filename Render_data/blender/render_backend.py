...
def setup_light(scene):
    ...
    azi = np.random.uniform(0, 120)
    ele = np.random.uniform(30, 45)
    dist = np.random.uniform(2, 2.5)
    x, y, z = obj_location(dist, azi, ele)
    lamp_name = 'Lamp{}'.format("1")
    #光对象设置
    lamp_data = bpy.data.lamps.new(name=lamp_name, type='AREA')

    lamp_data.energy = np.random.uniform(30, 40)
    lamp = bpy.data.objects.new(name=lamp_name, object_data=lamp_data)
    lamp.location = (x, y, z)
    scene.objects.link(lamp)

    azi = np.random.uniform(120, 240)
    ele = np.random.uniform(30, 45)
    dist = np.random.uniform(2, 2.5)
    x, y, z = obj_location(dist, azi, ele)
    lamp_name = 'Lamp{}'.format("2")
    #光对象设置
    lamp_data = bpy.data.lamps.new(name=lamp_name, type='AREA')
    ...
    azi = np.random.uniform(240, 360)
    ele = np.random.uniform(30, 45)
    dist = np.random.uniform(2, 2.5)
    x, y, z = obj_location(dist, azi, ele)
    lamp_name = 'Lamp{}'.format("3")
    #光对象设置
    lamp_data = bpy.data.lamps.new(name=lamp_name, type='AREA')
    ...
    #     dist = np.random.uniform(4, 6)
    #     x, y, z = obj_location(dist, azi, ele)
    #     lamp_name = 'Lamp{}'.format(i)
    #     #光对象设置
    #     lamp_data = bpy.data.lamps.new(name=lamp_name, type='AREA')
    #
    #     lamp_data.energy = np.random.uniform(2.5, 4.5)
    #     lamp = bpy.data.objects.new(name=lamp_name, object_data=lamp_data)
    #     lamp.location = (x, y, z)
    #     scene.objects.link(lamp)
...
def set_material_node_parameters(material):
    ...
    if os.path.basename(args.input).endswith('.ply'):
        nodes['Glossy BSDF'].inputs['Roughness'].default_value = np.random.uniform(0.2, 0.4)
    else:
        nodes['Diffuse BSDF'].inputs['Roughness'].default_value = np.random.uniform(0, 0.5)
