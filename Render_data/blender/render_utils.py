...
class DataStatistics(object):
    ...
    def __init__(self, class_type):
        ...
        self.dataset_poses_path = os.path.join(dataset_pose_dir_path, 'obj2_poses.npy'.format(class_type))
    ...
    def get_dataset_poses(self):
        ...
        for name in gt_name:
            gt = yaml.load(open(os.path.join(pose_path, name), 'r'))
            inds = list(gt.keys())
            train_inds = [int(ind.replace(".png", "")) for ind in inds]
            train_inds.sort()

            for idx in train_inds:
                img_name = '{}.png'.format(idx)
                for i in range(gt['1'].__len__()):
                    instance_gt = gt[str(idx)][i]
    ...
    def sample_poses(self):
        ...
        mean_x=np.mean(translations[:,0])
        mean_y=np.mean(translations[:,1])
        mean_z=np.mean(translations[:,2])
        for i in range(translations.shape[0]):
            translations[i,0]=mean_x
            translations[i,1]=mean_y
            translations[i,2]=mean_z/1.4
...
class Renderer(object):
    intrinsic_matrix = {
        ...
        #                      [0.0, 280.0, 128.0],
        #                      [0.0, 0.0, 1.0]]),
        'blender': np.array([[1173.308073739741, 0., 329.138849381171],
    ...
    def __init__(self, class_type):
        ...
        self.obj_path = os.path.join(cfg.LINEMOD,'cad/{}.ply').format( class_type)
        self.small_obj_path=os.path.join(cfg.LINEMOD,'cad/{}.ply').format( class_type)
    ...
    def run(self):
        ...
        os.system('{} {} --background --python {} -- --input {} --output_dir {} --bg_imgs {} --poses_path {}'.
                  format(self.blender_path, self.blank_blend, self.py_path, self.small_obj_path,
