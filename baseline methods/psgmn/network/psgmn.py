...
from visualize_test import pnp
import matplotlib
matplotlib.use('TKAgg')
import matplotlib.patches as patches
from matplotlib import pyplot as plt
from utils.utils import project
#from visualize_test import visualize_mask,visualize_bounding_box
import yaml
import os
index=1
class psgmn(torch.nn.Module):
    ...
    def __init__(self,mesh_dir,
        ...
        self.img_in_channels = img_in_channels   #64
        self.mesh_in_channels = mesh_in_channels  #9
        self.out_channels = out_channels            #64

        self.num_img_layers = num_img_layers    #2
        self.num_mesh_layers = num_mesh_layers  #6

        self.img_dim = img_dim  #2
        self.mesh_dim = mesh_dim  #3
    ...
    def resize_mat(self,img,mask,pose,K):
        ...
        for i in range(batch_size):
            ...
            img[i],mask[i],correspond_node_idx[i] = \
                self.crop_mat(\
                    img[i].clone().detach(),mask[i].clone().detach(),correspond_node_idx[i].clone().detach(),target_height,target_width)
                  
        return img,mask,correspond_node_idx
        
    def forward(self,x,mask=None,pose=None,K=None,*args,**kwargs):

        if self.training:
            x,mask,node_index = self.resize_mat(x.clone().detach(),mask.clone().detach(),pose,K)
        ...
        if self.training:
            ...
            bbox_img=kwargs['bbox_img']
            bbox_img=torch.from_numpy(bbox_img).cuda()
            mask_pred=mask_pred*bbox_img
            '''
            for i in range(mask_pred.shape[0]):

                mask_pred[i, :,:kwargs['bbox_x'][i]]=0
                mask_pred[i, :kwargs['bbox_y'][i], :] = 0
                mask_pred[i, :, kwargs['bbox_x'][i]+kwargs['bbox_w'][i]+1:] = 0
                mask_pred[i, kwargs['bbox_y'][i]+kwargs['bbox_h'][i]+1:, :] = 0
            '''
            idx = torch.where(mask_pred.reshape(-1))[0]
            
        img_feauture_selected = features.index_select(0,idx).permute(1,0)

        S = torch.matmul(meshes,img_feauture_selected) #mesh.shape=[3502,64] ,img_feauture_selected.shape=[64,9988]，S=[3502,9988]
        ...
        if self.training:
            ...
            #point_idx=valid_pts_ind.clone().detach().cpu().numpy()
