...
import sys
sys.path.append("/home/wzz/4T/chaoyue/surfemb")
...
parser.add_argument('--model_path', default= 'data/models/rtl-render.ckpt')
parser.add_argument('--scene_dir_id', default= '27')
...
scene_dir_id = str(args.scene_dir_id)
res_crop = args.res_crop
device = torch.device(args.device)
model_path = Path(args.model_path)
assert model_path.is_file()
model_name = model_path.name.split('.')[0]
dataset = model_name.split('-')[0] + '-true'

results_dir = Path('data/results')
results_dir.mkdir(exist_ok=True)
pose_dir = osp.join('data/results', scene_dir_id)
if not osp.exists(pose_dir):
    os.mkdir(pose_dir)
model_name_render = dataset.split('_')[0] + '-render'
poses_fp = results_dir / f'{scene_dir_id}/{model_name_render}-poses.npy'
poses_scores_fp = results_dir / f'{scene_dir_id}/{model_name_render}-poses-scores.npy'
poses_timings_fp = results_dir / f'{scene_dir_id}/{model_name_render}-poses-timings.npy'
...
data = detector_crops.DetectorCropDataset(
    ...
    detection_folder=Path(f'data/detection_results/{dataset}/{scene_dir_id}'),
